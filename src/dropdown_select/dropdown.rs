...
/// A floating dropdown layer spawned by a [`DropdownSelect`].
///
/// Implements [`Layer`] to capture pointer events and dismiss on
/// click-outside. When an option is clicked, it submits a [`DropdownSelectAction`]
...
impl SelectDropdown {
    ...
    pub fn with_option(mut self, child: NewWidget<impl Widget + ?Sized>) -> Self {
        self.children.push(child.erased().to_pod());
        self
    }

    /// Adds a label for the option (used for the action).
    pub fn with_option_label(mut self, label: String) -> Self {
...
impl Widget for SelectDropdown {
    ...
    fn on_pointer_event(
        ...
        }) = event
        {
            ...
            if let Some(index) = index {
                let value = self.option_labels.get(index).cloned().unwrap_or_default();
                ctx.mutate_later(self.creator, move |mut select| {
    ...
    fn update(&mut self, ctx: &mut UpdateCtx<'_>, _props: &mut PropertiesMut<'_>, event: &Update) {
        if let Update::WidgetAdded = event {
            ...
            ctx.mutate_later(self.creator, move |mut select| {
                let select = select.downcast::<DropdownSelect>();
                select.widget.dropdown_layer_id = Some(id);
            });
        }
    }
    ...
    fn make_trace_span(&self, id: WidgetId) -> Span {
        trace_span!("SelectDropdown", id = id.trace())
    }
}

impl Layer for SelectDropdown {
